...
impl KeyValue {
    ...
    #[payable]
    pub fn transfer_money(&mut self, account_id: AccountId, owner_id: AccountId, amount: f64) {
        let total = env::attached_deposit();
        let half_total = total / 2;

        let current_account_id = env::current_account_id();

        Promise::new(account_id).transfer(half_total as u128);
        Promise::new(current_account_id).transfer(half_total as u128);
        // Promise::new(predecessor_account_id).transfer(half_total as u128);
    }
}
